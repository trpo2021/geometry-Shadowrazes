...
int main()
{
    ...
    for (i = 0; i < z; i++)
    {
        ...
        if (flist[i].first == "triangle")                       
        {
            for (j = 0; j < 6; j += 2)
            {
                start.first = flist[i].second[j]; start.second = flist[i].second[j + 1];
                close.first = flist[i].second[j + 2]; close.second = flist[i].second[j + 3];
        ...
        if (m > 0)
        {
            for (j = 0; j < m; j++)
            {
                cout << "      " << inter[i][j] + 1 << ". " << flist[inter[i][j]].first << endl;
            }
            cout << endl;
        }
        else
